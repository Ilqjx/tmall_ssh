			...
			<tbody>
				<c:forEach items="${categorys}" var="c">
					<tr>
						<td>${c.id}</td>
						<td>
							<img src="img/category/${c.id}.jpg" height="40px" />
						</td>
						<td>${c.name}</td>
						<td>
						...
						<td>
							<a href="admin_category_edit?category.id=${c.id}">
								<span class="glyphicon glyphicon-edit"></span>
							</a>
						</td>
						<td>
							<a deleteLink="true" href="admin_category_delete?category.id=${c.id}">
			...
			<form id="addForm" action="admin_category_add" method="post" enctype="multipart/form-data">
				<table class="addTable">
					<tr>
						<td>分类名称</td>
						<td>
							<input id="name" class="form-control" type="text" name="category.name" />
						</td>
					</tr>
					<tr>
						<td>分类图片</td>
